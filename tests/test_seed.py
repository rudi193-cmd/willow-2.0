...
def test_step_4_vault_key_permissions(tmp_path, monkeypatch):
    ...
    wh.mkdir(parents=True, exist_ok=True)
    seed.step_4_vault()
    key_path = wh / "vault.key"  # canonical path from core/vault.py
...
def test_step_4_vault_idempotent(tmp_path, monkeypatch):
    ...
    wh.mkdir(parents=True, exist_ok=True)
    seed.step_4_vault()
    key1 = (wh / "vault.key").read_bytes()  # canonical path
    seed.step_4_vault()
    key2 = (wh / "vault.key").read_bytes()
    assert key1 == key2
