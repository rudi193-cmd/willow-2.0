...
```bash
psql -U example-user willow_19 -c "CREATE SCHEMA IF NOT EXISTS willow;"
psql -U example-user willow_19 -f <(cat <<'SQL'
CREATE TABLE IF NOT EXISTS willow.routing_decisions (
...
psql -U example-user willow_19_test -f <(cat <<'SQL'
CREATE TABLE IF NOT EXISTS willow.routing_decisions (
    id SERIAL PRIMARY KEY, ts TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    session_id TEXT, prompt_snippet TEXT, routed_to TEXT NOT NULL,
    rule_matched TEXT NOT NULL, confidence NUMERIC(4,3) NOT NULL DEFAULT 1.0,
...
```bash
psql -U example-user willow_19 -c "\d willow.routing_decisions"
```
...
```bash
echo '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"willow_route","arguments":{"message":"debug the gleipnir rate limit","session_id":"test"}}}' | PYTHONPATH=/home/example/github/willow-1.9 python3 /home/example/github/willow-1.9/sap/sap_mcp.py
```
