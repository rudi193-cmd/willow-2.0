...
PACKAGE_ROOT = Path("/home/example/github/willow-1.9")
...
```bash
python -m willow.fylgja.install --dry-run --settings /home/example/.claude/settings.json
```
...
```bash
python -m willow.fylgja.install --settings /home/example/.claude/settings.json
```
