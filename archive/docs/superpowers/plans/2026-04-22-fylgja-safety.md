...
def test_hs008_blocks_settings_overwrite_without_install():
    result = check_all(
        tool_name="Write",
        tool_input={"file_path": "/home/example/.claude/settings.json"},
        user_role="adult",
        training_consented=False,
    )
    assert result is not None
...
def test_hs008_allows_normal_file_write():
    result = check_all(
        tool_name="Write",
        tool_input={"file_path": "/home/example/agents/hanuman/learned/note.md"},
        user_role="adult",
        training_consented=False,
    )
    assert result is None
...
def test_clean_read_passes_all_stops():
    result = check_all(
        tool_name="Read",
        tool_input={"file_path": "/home/example/github/willow-1.9/README.md"},
        user_role="adult",
        training_consented=False,
    )
    assert result is None
...
_PROTECTED_PATHS = {
    ...
    "/home/example/.claude/settings.local.json",
}
