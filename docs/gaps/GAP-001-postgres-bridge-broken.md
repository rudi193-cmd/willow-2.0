...
`settings.json` had `WILLOW_PG_DB: "willow"`. The `willow` database is owned by the `willow` OS user, not `sean-campbell`. The correct database is `willow_20` (owned by `sean-campbell`). This made `PgBridge()` initialization fail when finally corrected in Failure 1 — the right variable type was pointing at the wrong DB.
