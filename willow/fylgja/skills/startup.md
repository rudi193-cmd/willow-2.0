...
2. **Postgres — live probe, not anchor copy.** The anchor field `postgres` can be `unknown` when SessionStart could not reach MCP. Do **not** report that as the truth. Prefer `willow_status`: `postgres` is a **dict** ⇒ up. If MCP is missing, times out, or returns non-dict: from the **willow-1.9 repo root** run `pg_isready` (honor `WILLOW_PG_HOST` / `WILLOW_PG_PORT` if set) and one connect + `SELECT 1` against `WILLOW_PG_DB` (default `willow_19`) — e.g. `python3 -c "from core.pg_bridge import try_connect; c=try_connect(); assert c; c.close()"` or `psql`. **Probe success** ⇒ treat as up for the boot paragraph. **Probe failure** ⇒ post `#general`, stop.
...
5. Grove LISTEN monitor — `willow/fylgja/skills/grove-persistent-monitor.md` (all msgs on own channel; `@mentions` elsewhere); never `last_id=0`. **Use Postgres LISTEN/NOTIFY only — never subprocess polling or HTTP grove_get_history loops.** Covers own channel (all) + `@agent`/`@all` tags fleet-wide.
...
7b. **#learnings pull** — `grove_get_history("learnings", limit=5)`. Skim entries since `anchor.written_at`. Extract one-line title per post not yet seen. Surface in report if any new since last session. Skip if none new.
7c. **Feedback principles** — `opus_feedback(app_id=<boot agent>)` — all entries, all domains. Read before report. Any principle whose domain matches today's work (drift, recurrence, monitor, meta) surfaces as a one-liner in the boot paragraph. These are durable behavioral rules extracted from the correction corpus — not suggestions.
